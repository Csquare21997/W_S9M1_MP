...
describe('Auth component', () => {
  ...
  test('[4] Submitting an empty form shows "Invalid Credentials" message', async () => {
    ...
    expect (screen.findByText ('Invalid Credentials')).toBeVisible()
  ...
  for (const usr of registeredUsers) {
    test(`[6.${usr.id}] Logging in ${usr.username} makes the following elements render:
      ...
      await user.click (loginBtn)
      await waitFor (() => {
        expect (screen.getByText (`Welcomee back, ${usr.username}, We LOVE you!`)).toBeVisible()
        expect (screen.getByText (`ID: ${user.id}, Username: ${user.username}, Born:${user.born}`)).toBeVisible()
